...
import {
  ...
  Mail, KeyRound, UserPlus, Loader2
} from 'lucide-react';
...
const STATUS = {
  open:     { label: 'Aperto',    color: '#ef4444', bg: 'rgba(239,68,68,0.12)',  ring: 'rgba(239,68,68,0.3)',  icon: AlertCircle },
  fixed:    { label: 'In verifica', color: '#f59e0b', bg: 'rgba(245,158,11,0.12)', ring: 'rgba(245,158,11,0.3)', icon: Clock },
  approved: { label: 'Approvato', color: '#10b981', bg: 'rgba(16,185,129,0.12)', ring: 'rgba(16,185,129,0.3)', icon: CheckCircle2 },
  rejected: { label: 'Rifiutato', color: '#9ca3af', bg: 'rgba(156,163,175,0.12)', ring: 'rgba(156,163,175,0.3)', icon: RotateCcw },
};
  ...
  const submit = async () => {
    ...
    try {
      if (mode === 'login') {
        ...
        if (!firstName.trim() || !lastName.trim()) throw new Error('Nome e cognome richiesti');
        if (!isLeonardoEmail && !company.trim()) throw new Error('Indica il nome della tua azienda');
        ...
        if (error) throw error;
        setInfo('Account creato. Accedi ora.');
        setMode('login');
        setPassword('');
      }
    } catch (e) {
          ...
          <div className="login-form">
            {mode === 'signup' && (
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <input value={lastName} onChange={e => setLastName(e.target.value)} placeholder="Cognome" autoComplete="family-name" />
              </div>
            )}
            <input value={email} onChange={e => setEmail(e.target.value)} placeholder="Email aziendale" type="email" autoComplete="email" onKeyDown={e => e.key === 'Enter' && submit()} />
            ...
            {mode === 'signup' && !isLeonardoEmail && email.includes('@') && (
              <input value={company} onChange={e => setCompany(e.target.value)} placeholder="Nome azienda (es. i4 industry)" />
            )}
            {error && <div className="login-error">{error}</div>}
            {info && <div style={{ color: '#10b981', fontSize: 13 }}>{info}</div>}
            <button className="btn-login" onClick={submit} disabled={busy}>
              {busy ? <Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }}/> : (mode === 'login' ? 'ACCEDI' : 'REGISTRATI')}
            </button>
            <button
...
function MainApp({ me, onLogout }) {
  ...
  async function createSnag(data) {
    try {
      ...
      if (error) throw error;
      showToast(`Snag ${data.code} creato`, 'success');
      await loadSnags();
  ...
  async function resolveSnag(snag, resolutionPhotoDataUrl, note) {
    try {
      ...
      if (error) throw error;
      showToast('Risoluzione inviata', 'success');
      await loadSnags();
    } catch (e) {
      showToast('Errore: ' + (e.message || e), 'error');
      console.error(e);
...
function TopBar({ screen, params, user, canGoBack, onBack, onLogout, projects, snags }) {
  const title = useMemo(() => {
    if (screen === 'projects') return 'Progetti';
    if (screen === 'project') { const p = projects.find(x => x.id === params.projectId); return p?.name || 'Progetto'; }
    if (screen === 'newSnag') return 'Nuovo Snag';
    if (screen === 'snag') { const s = snags.find(x => x.id === params.snagId); return s ? s.code : 'Snag'; }
    if (screen === 'newProject') return 'Nuovo Progetto';
    if (screen === 'projectSettings') return 'Impostazioni';
    if (screen === 'users') return 'Utenti';
    return 'LEOFIELD';
  ...
  return (
    <div className="topbar">
      {canGoBack ? (
        <button className="icon-btn" onClick={onBack} aria-label="Indietro"><ArrowLeft size={20} /></button>
      ) : (
      ...
      <button className="icon-btn" onClick={onLogout} aria-label="Esci" title="Esci"><LogOut size={18} /></button>
    </div>
  );
}
    ...
    <div style={{ paddingTop: 20 }}>
      ...
      {visible.length === 0 ? (
        <div className="empty">
          ...
          {isLeo && <div style={{ fontSize: 13, marginTop: 4 }}>Crea il primo progetto per iniziare.</div>}
        </div>
      ) : (
                ...
                <div className="card-row" style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: 10, marginTop: 8, fontSize: 12 }}>
                      {open > 0 && <span style={{ color: '#ef4444' }}>● {open} aperti</span>}
                      {fixed > 0 && <span style={{ color: '#f59e0b' }}>● {fixed} in verifica</span>}
                      {open === 0 && fixed === 0 && <span style={{ color: 'var(--text-faint)' }}>Nessuno snag</span>}
                    </div>
                  </div>
...
function ProjectScreen({ user, project, allSnags, users, onOpenSnag, onNewSnag, onSettings, onShowToast }) {
  ...
  if (!project) return <div className="empty" style={{ marginTop: 24 }}>Progetto non trovato.</div>;
  ...
  const exportExcel = () => {
    try {
      const rows = visible.map(s => {
        ...
        return {
          'Codice': s.code, 'Stato': STATUS[s.status]?.label || s.status,
          'Titolo': s.title, 'Descrizione': s.description || '',
          'Creato da': creator ? `${creator.first_name} ${creator.last_name}` : '',
          'Azienda creatore': creator?.company || '',
          'Data creazione': s.created_at ? new Date(s.created_at).toLocaleString('it-IT') : '',
          'Assegnato a': s.assigned_company || '',
          'Risolto da': resolver ? `${resolver.first_name} ${resolver.last_name}` : '',
          'Data risoluzione': s.resolution?.at ? new Date(s.resolution.at).toLocaleString('it-IT') : '',
          'Note risoluzione': s.resolution?.note || '',
          'Approvato da': approver ? `${approver.first_name} ${approver.last_name}` : '',
          'Data approvazione': s.approval?.at ? new Date(s.approval.at).toLocaleString('it-IT') : '',
          'Coord. layout X (%)': s.mark ? (s.mark.x * 100).toFixed(1) : '',
          'Coord. layout Y (%)': s.mark ? (s.mark.y * 100).toFixed(1) : '',
        };
      });
      if (rows.length === 0) { onShowToast('Nessun dato da esportare', 'info'); return; }
      ...
      onShowToast(`Esportati ${rows.length} snag`, 'success');
    } catch (e) { onShowToast('Errore esportazione', 'error'); }
  };
    ...
    <div style={{ paddingTop: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 11, letterSpacing: '0.15em', color: 'var(--text-faint)', textTransform: 'uppercase', fontWeight: 600 }}>Progetto</div>
          <div style={{ fontWeight: 700, fontSize: 22, letterSpacing: '-0.01em' }}>{project.name}</div>
        </div>
        <div style={{ display: 'flex', gap: 4 }}>
          <button className="icon-btn" onClick={exportExcel} title="Esporta Excel"><Download size={18}/></button>
          {isLeo && <button className="icon-btn" onClick={onSettings}><Settings size={18}/></button>}
        </div>
      </div>

      <div style={{ display: 'flex', gap: 10, marginBottom: 18 }}>
        <div className="stat-card"><div className="stat-num" style={{ color: '#ef4444' }}>{counts.open}</div><div className="stat-lbl">Aperti</div></div>
        <div className="stat-card"><div className="stat-num" style={{ color: '#f59e0b' }}>{counts.fixed}</div><div className="stat-lbl">In verifica</div></div>
        <div className="stat-card"><div className="stat-num" style={{ color: '#10b981' }}>{counts.approved}</div><div className="stat-lbl">Approvati</div></div>
      </div>
      ...
      {openOnes.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 10, letterSpacing: '0.15em', color: 'var(--text-faint)', textTransform: 'uppercase', fontWeight: 600, marginBottom: 6 }}>
            <Target size={11} style={{ verticalAlign: 'middle', marginRight: 4 }}/>Punti aperti · {openOnes.length}
          </div>
      ...
      {layoutUrl && visible.some(s => s.mark) && (
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 10, letterSpacing: '0.15em', color: 'var(--text-faint)', textTransform: 'uppercase', fontWeight: 600, marginBottom: 6 }}>
            <MapPin size={11} style={{ verticalAlign: 'middle', marginRight: 4 }}/>Mappa punti
          </div>
      ...
      <div className="tab-bar" style={{ marginBottom: 16 }}>
        {['all', 'open', 'fixed', 'approved'].map(k => (
          <button key={k} className={filter === k ? 'active' : ''} onClick={() => setFilter(k)}>
            {k === 'all' ? 'Tutti' : STATUS[k]?.label} <span className="mono" style={{ opacity: 0.6 }}>{counts[k]}</span>
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="empty">
          ...
          <div>Nessuno snag {filter !== 'all' ? STATUS[filter]?.label.toLowerCase() : ''}.</div>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      {isLeo && (
        <div className="fab">
          <button onClick={onNewSnag} aria-label="Nuovo snag"><Plus size={24}/></button>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="layout-canvas">
      ...
      <svg viewBox="0 0 100 100" preserveAspectRatio="none" style={{ pointerEvents: 'none' }}>
        {snags.map(s => (
          <g key={s.id} style={{ pointerEvents: 'all', cursor: 'pointer' }} onClick={() => onOpenSnag(s.id)}>
...
function LayoutPicker({ image, mark, onChange, readOnly = false }) {
  ...
  return (
    <div ref={ref} className={`layout-canvas ${readOnly ? '' : 'interactive'}`}
         onClick={setFromEvent}
         onTouchStart={(e) => { e.preventDefault(); setFromEvent(e); }}>
      <img src={image} alt="layout" />
      {mark && (
...
function NewSnagScreen({ user, project, allSnags, users, onCancel, onCreate }) {
      ...
      <div style={{ paddingTop: 24 }}>
        <div className="card card-row">
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            ...
            <div>
              <div style={{ fontWeight: 600, marginBottom: 4 }}>Layout mancante</div>
              <div style={{ fontSize: 13, color: 'var(--text-dim)' }}>Carica un layout dalle impostazioni del progetto prima di creare snag.</div>
            </div>
          </div>
        </div>
        <button className="btn btn-ghost" onClick={onCancel} style={{ marginTop: 16, width: '100%' }}>Indietro</button>
      </div>
    );
  }
  ...
  return (
    <div style={{ paddingTop: 20 }}>
      ...
      <div style={{ fontSize: 11, color: 'var(--text-faint)', letterSpacing: '0.15em', textTransform: 'uppercase', fontWeight: 600, marginBottom: 8 }}>Passo {step} di 5</div>

      {step === 1 && (
        <div className="fadeIn">
          <h2 style={{ fontSize: 22, margin: '0 0 16px' }}>Scatta una foto della problematica</h2>
          <div style={{ marginBottom: 16 }}>
            {photo ? (
              <div style={{ position: 'relative' }}>
                <img src={photo} alt="" style={{ width: '100%', borderRadius: 12, border: '1px solid var(--border)' }} />
                <button onClick={() => setPhoto(null)} className="btn btn-ghost" style={{ position: 'absolute', top: 8, right: 8, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(10px)' }}><RotateCcw size={14}/> Riscatta</button>
              </div>
            ) : (
              <label style={{ display: 'block', cursor: 'pointer' }}>
                <div style={{ aspectRatio: '4/3', border: '2px dashed var(--border-strong)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 8, color: 'var(--text-dim)', background: 'var(--bg-2)' }}>
                  <Camera size={40} />
                  <div style={{ fontWeight: 600 }}>{busy ? 'Elaborazione…' : 'Scatta o carica foto'}</div>
                </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn btn-ghost" onClick={onCancel} style={{ flex: 1 }}>Annulla</button>
            <button className="btn btn-primary" onClick={() => setStep(2)} disabled={!photo} style={{ flex: 2 }}>Continua</button>
          </div>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="fadeIn">
          <h2 style={{ fontSize: 22, margin: '0 0 8px' }}>Segna il punto sulla foto</h2>
          <p style={{ color: 'var(--text-dim)', margin: '0 0 16px', fontSize: 14 }}>Tocca la foto per piazzare una <strong style={{ color: '#ef4444' }}>X</strong> sul punto preciso.</p>
          <PhotoMarkup image={photo} mark={photoMark} onChange={setPhotoMark}/>
          <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
            <button className="btn btn-ghost" onClick={() => setStep(1)} style={{ flex: 1 }}>Indietro</button>
            <button className="btn btn-primary" onClick={() => setStep(3)} disabled={!photoMark} style={{ flex: 2 }}>Continua</button>
          </div>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="fadeIn">
          <h2 style={{ fontSize: 22, margin: '0 0 16px' }}>Descrivi il problema</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 20 }}>
            <div><label>Titolo breve</label><input value={title} onChange={e => setTitle(e.target.value)} placeholder="es. Rullo bloccato zona carico" maxLength={80}/></div>
            <div><label>Descrizione</label><textarea value={description} onChange={e => setDescription(e.target.value)} rows={5} placeholder="Contesto, note di sicurezza…" style={{ resize: 'vertical' }}/></div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn btn-ghost" onClick={() => setStep(2)} style={{ flex: 1 }}>Indietro</button>
            <button className="btn btn-primary" onClick={() => setStep(4)} disabled={!title.trim()} style={{ flex: 2 }}>Continua</button>
          </div>
        </div>
      )}
      ...
      {step === 4 && (
        <div className="fadeIn">
          <h2 style={{ fontSize: 22, margin: '0 0 8px' }}>Segna la posizione sul layout</h2>
          <p style={{ color: 'var(--text-dim)', margin: '0 0 16px', fontSize: 14 }}>Tocca il layout nel punto corrispondente.</p>
          {layoutUrl ? <LayoutPicker image={layoutUrl} mark={mark} onChange={setMark} /> : <div className="empty">Caricamento layout…</div>}
          <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
            <button className="btn btn-ghost" onClick={() => setStep(3)} style={{ flex: 1 }}>Indietro</button>
            <button className="btn btn-primary" onClick={() => setStep(5)} disabled={!mark} style={{ flex: 2 }}>Continua</button>
          </div>
        </div>
      )}
      ...
      {step === 5 && (
        <div className="fadeIn">
          <h2 style={{ fontSize: 22, margin: '0 0 16px' }}>Assegna e invia</h2>
          <div style={{ marginBottom: 20 }}>
            <label>Assegna al fornitore</label>
          ...
          <div className="card" style={{ marginBottom: 20 }}>
            <div className="card-row">
              <div style={{ fontSize: 11, color: 'var(--text-faint)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 8, fontWeight: 600 }}>Riepilogo</div>
              <div style={{ fontWeight: 600, marginBottom: 6 }}>{title}</div>
              {description && <div style={{ fontSize: 13, color: 'var(--text-dim)', marginBottom: 10 }}>{description}</div>}
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn btn-ghost" onClick={() => setStep(4)} style={{ flex: 1 }}>Indietro</button>
            <button className="btn btn-primary" onClick={submit} style={{ flex: 2 }}><Check size={16}/> Crea snag</button>
          </div>
        </div>
      )}
    </div>
...
function SnagScreen({ user, users, projects, snag, onBack, onResolve, onApprove, onReject, onDelete }) {
  ...
  if (!snag) return <div className="empty" style={{ marginTop: 24 }}>Snag non trovato.</div>;
  ...
  return (
    <div style={{ paddingTop: 20 }}>
      ...
      <div style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 20 }}>
        Creato da {creator ? `${creator.first_name} ${creator.last_name}` : '—'} · {fmtDate(snag.created_at)}<br />
        Assegnato a <span style={{ color: 'var(--accent)' }}>{snag.assigned_company}</span>
      </div>

      {snag.description && (
        <div className="card card-row" style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 11, color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600, marginBottom: 6 }}>Descrizione</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginBottom: 16 }}>
        <div>
          <div style={{ fontSize: 11, color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600, marginBottom: 8 }}>Foto problematica</div>
          {photoUrl ? <PhotoMarkup image={photoUrl} mark={snag.photo_mark} onChange={() => {}} readOnly /> : <div className="empty">Caricamento foto…</div>}
        </div>
        {layoutUrl && (
          <div>
            <div style={{ fontSize: 11, color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600, marginBottom: 8 }}>Posizione sul layout</div>
      ...
      {snag.resolution && (
        <div className="card" style={{ marginBottom: 16, borderColor: 'rgba(245,158,11,0.3)' }}>
          <div style={{ padding: 14, borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8 }}>
            <Clock size={16} color="#f59e0b"/><div style={{ fontWeight: 600 }}>Risoluzione caricata</div>
          </div>
          <div className="card-row">
            {resolutionUrl ? <img src={resolutionUrl} alt="" style={{ width: '100%', borderRadius: 8, border: '1px solid var(--border)' }} /> : <div className="empty">Caricamento…</div>}
      ...
      {snag.approval && snag.status === 'approved' && (
        <div className="card" style={{ marginBottom: 16, borderColor: 'rgba(16,185,129,0.3)' }}>
          <div style={{ padding: 14, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 600 }}>Approvato</div>
      ...
      {canResolve && !showResolveForm && (
        <button className="btn btn-primary" onClick={() => setShowResolveForm(true)} style={{ width: '100%', marginBottom: 12 }}>
          <Camera size={16}/> Segna come risolto e carica foto
        </button>
      )}

      {canResolve && showResolveForm && (
        <div className="card card-row fadeIn" style={{ marginBottom: 12 }}>
          <div style={{ fontWeight: 600, marginBottom: 12 }}>Carica foto della risoluzione</div>
          {resolutionPhoto ? (
            ...
            <label style={{ display: 'block', cursor: 'pointer', marginBottom: 12 }}>
              <div style={{ aspectRatio: '4/3', border: '2px dashed var(--border-strong)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 8, color: 'var(--text-dim)' }}>
                <Camera size={32}/><div>{busy ? 'Elaborazione…' : 'Scatta foto'}</div>
              </div>
              <input type="file" accept="image/*" capture="environment" onChange={e => handleResolvePhoto(e.target.files?.[0])} style={{ display: 'none' }}/>
            </label>
          )}
          <label>Note (opzionale)</label>
          <textarea value={resolutionNote} onChange={e => setResolutionNote(e.target.value)} rows={3} placeholder="Come è stato risolto…" style={{ resize: 'vertical' }} />
          <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
            <button className="btn btn-ghost" onClick={() => { setShowResolveForm(false); setResolutionPhoto(null); }} style={{ flex: 1 }}>Annulla</button>
            <button className="btn btn-primary" onClick={submitResolution} disabled={!resolutionPhoto} style={{ flex: 2 }}>Invia per approvazione</button>
          </div>
        </div>
      )}
      ...
      {canApprove && !showRejectForm && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          <button className="btn btn-danger" onClick={() => setShowRejectForm(true)} style={{ flex: 1 }}><X size={16}/> Rifiuta</button>
          <button className="btn btn-success" onClick={() => onApprove(snag)} style={{ flex: 2 }}><Check size={16}/> Approva</button>
        </div>
      )}

      {canApprove && showRejectForm && (
        <div className="card card-row fadeIn" style={{ marginBottom: 12 }}>
          <div style={{ fontWeight: 600, marginBottom: 8 }}>Motivo del rifiuto</div>
          <textarea value={rejectNote} onChange={e => setRejectNote(e.target.value)} rows={3} placeholder="Perché la risoluzione non è accettabile…"/>
          <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
            <button className="btn btn-ghost" onClick={() => setShowRejectForm(false)} style={{ flex: 1 }}>Annulla</button>
            <button className="btn btn-danger" onClick={submitReject} disabled={!rejectNote.trim()} style={{ flex: 2 }}>Rifiuta e riapri</button>
          </div>
        </div>
      )}
      ...
      {snag.history && snag.history.length > 0 && (
        <div className="card" style={{ marginTop: 20 }}>
          <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)', fontSize: 11, color: 'var(--text-faint)', letterSpacing: '0.15em', textTransform: 'uppercase', fontWeight: 600 }}>Storico</div>
      ...
      {isLeo && (
        <button className="btn btn-danger" onClick={() => { if (confirm('Eliminare definitivamente questo snag?')) onDelete(snag.id); }} style={{ width: '100%', marginTop: 20 }}>
          <Trash2 size={14}/> Elimina snag
        </button>
      )}
    </div>
  );
...
function actionLabel(a) {
  return ({ created: 'ha creato lo snag', resolved: 'ha caricato la risoluzione', approved: 'ha approvato', rejected: 'ha rifiutato, snag riaperto' })[a] || a;
}
...
function NewProjectScreen({ user, users, onCancel, onCreate }) {
  ...
  return (
    <div style={{ paddingTop: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div><label>Nome progetto</label><input value={name} onChange={e => setName(e.target.value)} placeholder="es. Amazon BHX2"/></div>
      <div><label>Descrizione</label><textarea value={description} onChange={e => setDescription(e.target.value)} rows={2}/></div>
      <div>
        <label>Layout impianto</label>
        {layoutImage ? (
          <div style={{ position: 'relative' }}>
          ...
          <>
            <label style={{ cursor: 'pointer', display: 'block' }}>
              <div style={{ aspectRatio: '16/9', border: '2px dashed var(--border-strong)', borderRadius: 10, display: 'flex', alignItems:'center', justifyContent:'center', gap: 8, flexDirection: 'column', color: 'var(--text-dim)' }}>
                <Upload size={28}/><div>{busy ? 'Elaborazione…' : 'Carica layout'}</div>
              </div>
              <input type="file" accept="image/*" onChange={e => handleLayout(e.target.files?.[0])} style={{ display: 'none' }}/>
            </label>
            <button className="btn btn-ghost" onClick={useBhx2Preset} style={{ marginTop: 8, width: '100%', fontSize: 12 }}>Usa layout demo BHX2</button>
          </>
        )}
      </div>
      <div>
        ...
        {vendorCompanies.length === 0 ? (
          <div style={{ fontSize: 13, color: 'var(--text-dim)', padding: 12, border: '1px dashed var(--border)', borderRadius: 8 }}>
            Nessun fornitore registrato. Condividi il link dell'app con i tuoi fornitori: quando si registreranno potrai aggiungerli ai progetti.
          </div>
        ) : (
...
function ProjectSettingsScreen({ project, user, users, onBack, onUpdate, onDelete }) {
  ...
  return (
    <div style={{ paddingTop: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {displayedLayout ? (
          <div style={{ position: 'relative' }}>
            ...
            <label style={{ position: 'absolute', top: 8, right: 8, cursor: 'pointer' }}>
              <div className="btn btn-ghost" style={{ background: 'rgba(0,0,0,0.6)'}}><Edit3 size={14}/> Sostituisci</div>
              <input type="file" accept="image/*" onChange={e => handleLayout(e.target.files?.[0])} style={{ display: 'none' }}/>
            </label>
          </div>
        ) : (
      ...
      <button className="btn btn-primary" onClick={save}>Salva modifiche</button>
      <button className="btn btn-danger" onClick={() => { if (confirm(`Eliminare "${project.name}" e tutti i suoi snag?`)) onDelete(project.id); }}>
        <Trash2 size={14}/> Elimina progetto
      </button>
    </div>
  );
}
...
function UsersScreen({ user, users, onBack, onShowToast }) {
  ...
  return (
    <div style={{ paddingTop: 20 }}>
      <div className="card card-row" style={{ marginBottom: 20, background: 'var(--accent-soft)', borderColor: 'rgba(227,6,19,0.3)' }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          ...
          <div style={{ fontSize: 13, lineHeight: 1.5 }}>
            <div style={{ fontWeight: 600, marginBottom: 4, color: 'var(--text)' }}>Come aggiungere utenti</div>
            Condividi l'URL dell'app. I nuovi utenti si registrano da soli dalla pagina di login. Chi usa email <strong>@leonardo.com</strong> diventa automaticamente admin Leonardo. Gli altri sono fornitori e indicano la loro azienda.
          </div>
        </div>
      </div>
      ...
      <div style={{ fontSize: 11, letterSpacing: '0.15em', color: 'var(--text-faint)', textTransform: 'uppercase', marginBottom: 10, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
        <Building2 size={12}/> Fornitori ({vendors.length})
      </div>
...
function UserRow({ u, currentUser }) {
  ...
  return (
    <div className="card card-row" style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 600, fontSize: 14 }}>{u.first_name} {u.last_name} {isMe && <span style={{ color: 'var(--text-faint)', fontSize: 11, fontWeight: 400 }}>· tu</span>}</div>
